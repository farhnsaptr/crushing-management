...
export class VerificationService {
  ...
  static async getVerificationDetails(dateStr: string, shift: 'Pagi' | 'Malam') {
    ...
    // 4. If existing verification items exist in DB, merge box_count & kg_per_box
    if (existingHeader) {
      ...
      for (const sItem of savedItems) {
        ...
        entry.kg_per_box = Number(sItem.kg_per_box) || 5.0;
        entry.actual_output_kg = Number(sItem.actual_output_kg) || entry.box_count * entry.kg_per_box;
    ...
    const items = Array.from(materialMap.values()).map((item) => {
      ...
      const kgPerBox = Number(item.kg_per_box) || 5.0;
      const actualOut = Number((boxCount * kgPerBox).toFixed(2));
  ...
  static async saveVerification(
    ...
    // 1. Calculate totals & Validate that actual_output_kg does NOT exceed system_total_weight_kg
    ...
    const processedItems = items.map((item) => {
      ...
      const boxCount = Math.max(0, Number(item.box_count) || 0);
      const kgPerBox = Number(item.kg_per_box) > 0 ? Number(item.kg_per_box) : 5.0;
      const actOut = Number((boxCount * kgPerBox).toFixed(2));

      // Strict Validation: Output box weight cannot exceed system weight!
      if (actOut > sysTot) {
        const excess = (actOut - sysTot).toFixed(2);
        throw new Error(
          `Gagal Validasi: Total output box material "${item.material_name.trim()}" (${actOut.toFixed(2)} kg) melebihi akumulasi berat sistem (${sysTot.toFixed(2)} kg) sebesar ${excess} kg! Hasil output fisik tidak boleh melebihi input sistem.`
        );
      }
    ...
    for (const item of processedItems) {
      ...
      await pool.query(
        `INSERT INTO input_verification_items
         (id, verification_id, material_id, material_name_snapshot, system_ng_weight_kg, system_runner_weight_kg, system_total_weight_kg, box_count, kg_per_box, crushing_waste_kg)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          itemId,
          verificationId,
  ...
  static async getDashboardVerificationStatus(dateStr?: string, shift?: string) {
    ...
    if (rows.length === 0 || rows[0].status !== 'validated') {
      return {
        ...
        is_validated: false,
        status: 'pending',
        message: `Status Pekerjaan Tanggal ${targetDate} (${targetShift}) BELUM DIVALIDASI di dunia nyata oleh Operator.`,
        header: null,
      };
    }
